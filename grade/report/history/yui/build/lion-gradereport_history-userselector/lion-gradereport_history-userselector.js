YUI.add('lion-gradereport_history-userselector', function (Y, NAME) {
 ...
 *
 * @module     lion-gradereport_history-userselector
 * @main       lion-gradereport_history-userselector
 */
...
Y.namespace('M.gradereport_history').UserSelector = Y.extend(USERSELECTOR, M.core.dialogue, {
    ...
    initializer: function() {
                    ...
                    '<div class="{{CSS.SEARCH}}" role="search">' +
                        '<form>' +
                            '<input type="text" class="{{CSS.SEARCHFIELD}}" ' +
                                'aria-label="{{get_string "search" "lion"}}" value="" />' +
                            '<input type="submit" class="{{CSS.SEARCHBTN}}"' +
                                'value="{{get_string "search" "lion"}}">' +
                        '</form>' +
        ...
        this.get('boundingBox').one('.lion-dialogue-wrap').addClass('lion-dialogue-content');
    ...
    processSearchResults: function(tid, outcome, args) {
        ...
        if (!args.append) {
            if (totalUsers === 0) {
                bb.one(SELECTORS.RESULTSCOUNT).setHTML(M.util.get_string('noresults', 'lion'));
                content = '';
            } else {
                if (totalUsers === 1) {
