 ...
 */
abstract class lionform_mod extends lionform {
    /** Current data */
    protected $current;
    /**
    ...
    function lionform_mod($current, $section, $cm, $course) {
        global $CFG;
    ...
    /**
     * Only available on lionform_mod.
     *
    ...
    function standard_coursemodule_elements(){
        ...
        if ($this->_features->rating) {
            ...
            if (!empty($this->_cm)) {
                ...
                $rolenames = get_role_names_with_caps_in_context($context, array('lion/rating:rate', 'mod/'.$this->_cm->modname.':rate'));
        ...
        if (!empty($this->_cm)) {
            $context = context_module::instance($this->_cm->id);
            if (!has_capability('lion/course:activityvisibility', $context)) {
