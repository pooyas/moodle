...
if ($PAGE->user_allowed_editing() ) {
    $button = $OUTPUT->edit_button(new lion_url("$CFG->wwwroot/tag/index.php", array('id' => $tag->id)));
}
...
if ($tag->flag > 0 && has_capability('lion/tag:manage', $systemcontext)) {
...
if (!empty($CFG->enableblogs) && has_capability('lion/blog:view', $systemcontext)) {
    require_once($CFG->dirroot.'/blog/lib.php');
...
if (!empty($blogs)) {
    ...
    $allblogsurl = new lion_url('/blog/index.php', array('tagid' => $tag->id));
...
if ($usercount > 0) {
    ...
    $baseurl = new lion_url('/tag/index.php', array('id' => $tag->id));
