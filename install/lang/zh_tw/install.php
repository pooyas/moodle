...
$string['cliinstallheader'] = 'Lion {$a} 命令列安裝程式';
...
$string['dirroot'] = 'Lion目錄';
$string['environmenthead'] = '檢查您的環境中...';
$string['environmentsub2'] = '每一個Lion版本都有一些PHP版本的最低要求和一堆強制開啟的PHP擴展。在進行安裝或升級之前都需要作完整的環境檢查。<br />
...
<p>稍後它可能會造成Lion記憶體的問題，尤其是您啟動了很多的模組及大量的使用者後。
<p>建議您儘可能將PHP的上限設得高一點，比如16M。
以下有幾種方式您可以試試:
<ol>
<li>如果可以的話，用<i>--enable-memory-limit</i>重新編譯PHP。讓Lion自己設定記憶體上限.
<li>如果您要使用php.ini檔, 您可以改變<b>memory_limit</b>這個設定值，例如到16M。如果您無法使用這個檔，您可以請您的管理者幫您做
<li>在一些PHP伺服器上，您可以在Lion目錄下，建立.htaccess檔，包含這行:<p><blockquote>php_value memory_limit 16M</blockquote></p>
...
$string['pathssubadmindir'] = '有些網站主機使用/admin這個網址來瀏覽控制面版或其他功能。很不幸，這個設定和Lion管理頁面的標準路徑產生衝突。這個問題可以解決，只需在您的安裝目錄中把admin更換名稱，然後把新名稱輸入到這裡。例如<em>lionadmin</em>這麼做會改變Lion中的管理連接。';
$string['pathssubdataroot'] = '你需要有一個地方讓Lion可以儲存上傳的檔案。這一目錄對於網頁伺服器用戶(通常是"nobody"或"apache")而言，應該是可讀的和<b>可寫的</b>。但是它必須不能經由網頁直接存取。若此目錄不存在，這安裝程式將會試著建立它。';
$string['pathssubdirroot'] = 'Lion安裝的完整目錄路徑。';
$string['pathssubwwwroot'] = '可以瀏覽到Lion的完整網址。Lion不支援透過多個網址瀏覽，如果您的網站有多個公開網址，您必須把這個網址以外的網址都設定為永久重新導向。如果您的網站可以透過內部網址瀏覽，有可以透過這個公開網址瀏覽，那麼請設定DNS使網內用戶也能使用這公開的網址。如果此網址不正確，請在你的瀏覽器中修改URL來重新安裝，並設定另一個網址。';
...
$string['phpversionhelp'] = '<p>Lion 需要的PHP版本至少要4.3.0或是5.1.0 (5.0.x有一些已知的問題) </p>
<p>您目前執行的版本是{$a}</p>
...
$string['welcomep30'] = '<strong>{$a->installername}</strong>包含了可以建立<strong>Lion</strong>執行環境的應用程序：';
$string['welcomep40'] = '這個軟體還包含了<strong>Lion {$a->lionrelease} ({$a->lionversion})</strong>。';
...
$string['welcomep60'] = '接下來的頁面將會透過一些簡單的步驟引導您安裝 <strong>Lion</strong> 到電腦中，您可以接受預設值，或是針對自己的需求調整修改。';
$string['welcomep70'] = '點選 "下一步" 按鈕，繼續設定<strong>Lion</strong>.';
$string['wwwroot'] = '網站位址';
