...
$string['clialreadyconfigured'] = 'Файл config.php уже существует. Если Вы хотите установить Lion на этот сайт, используйте admin/cli/install_database.php.';
$string['clialreadyinstalled'] = 'Файл config.php уже существует. Если Вы хотите обновить сайт Lion, то используйте скрипт admin/cli/upgrade.php.';
$string['cliinstallheader'] = 'Программа установки Lion {$a} в режиме командной строки';
...
$string['dirroot'] = 'Каталог Lion';
$string['environmenthead'] = 'Проверка среды...';
$string['environmentsub2'] = 'У каждой версии Lion есть минимальные требования к версии PHP и набору обязательных расширений PHP.
...
<p>Из-за этого у через какое-то время у Lion могут возникнуть проблемы с памятью, особенно если у Вас будет много модулей и/или пользователей.</p>
...
В этом случае Lion сможет самостоятельно установить ограничение памяти.</li>
...
<li>На некоторых серверах PHP можно создать в каталоге Lion файл .htaccess содержащий следующую строку:
...
$string['pathssubadmindir'] = 'На небольшом числе веб-хостингов путь /admin используется для доступа к панели управления или чему-то еще. К сожалению, это противоречит стандартному расположению страниц управления Lion. Это можно исправить путем переименования папки admin в каталоге Lion и указания нового имени здесь. Например: <em>lionadmin</em>. При этом все ссылки на панель управления Lion исправятся автоматически.';
$string['pathssubdataroot'] = '<p>Каталог, в котором Lion будет хранить все файлы, размещаемые пользователями. </p><p>Этот каталог должен быть доступен для чтения и ЗАПИСИ тому пользователю, от чьего имени запускается веб-сервер (обычно \'www-data\', \'nobody\' или \'apache\'). </p><p>Этот каталог не должен быть доступен напрямую через Интернет. </p><p>Программа установки попробует создать этот каталог, если он не существует. </p>';
$string['pathssubdirroot'] = '<p>Полный путь к каталогу установки Lion.</p>';
$string['pathssubwwwroot'] = '<p>Полный веб-адрес, по которому будет доступен Lion, т.е. адрес, который пользователи будут вводить в адресной строке своего браузера для доступа к сайту Lion.</p>
<p>Использовать для доступа к Lion несколько публичных адресов невозможно. Если у Вашего сайта есть еще несколько публичных адресов, то следует настроить постоянные перенаправления с этих адресов на указанный.</p>
...
$string['phpversionhelp'] = '<p>Для Lion необходим PHP версии 4.3.0 и выше или 5.1.0 и выше (известны некоторые проблемы с версией 5.0.x).</p>
...
$string['welcomep30'] = 'Эта версия набора программ <strong>{$a->installername}</strong> включает следующие программы, необходимые для создания среды, в которой будет работать <strong>Lion</strong>:';
$string['welcomep40'] = 'Также в этот набор входит <strong>Lion {$a->lionrelease} ({$a->lionversion})</strong>.';
...
$string['welcomep60'] = 'На следующих страницах Вы сможете за несколько простых шагов настроить и установить <strong>Lion</strong> на свой компьютер. Вы сможете принять настройки по умолчанию или изменить их в зависимости от своих потребностей.';
$string['welcomep70'] = 'Нажмите кнопку «Далее» чтобы продолжить процесс установки <strong>Lion</strong>.';
