...
/// Class Functions
     ...
     */
    function is_empty() {
        if ( !has_capability('lion/block:view', $this->context) ) {
            return true;
        }
...
/*        if ($this->get_content() === NULL) {
     ...
     *
     * @deprecated since Lion 2.9 MDL-49385 - Please use Admin Settings functionality to save block configuration.
     * @todo MDL-49553 This will be deleted in Lion 3.1
     * @param array $data
     * @return boolean
     */
     ...
     * @param lion_page $the page this block is on.
     */
    ...
    function user_can_edit() {
        ...
        if (has_capability('lion/block:edit', $this->context)) {
            return true;
        }
        ...
        if (!empty($USER->id)
            ...
            return has_capability('lion/my:manageblocks', $this->page->context);
        }
        ...
        if (!empty($USER->id)
            ...
            if ((isset($formats['my']) && $formats['my'] == false)
                ...
                return $this->has_add_block_capability($page, $capability)
                       && has_capability('lion/my:manageblocks', $page->context);
            }
        }

        $capability = 'block/' . $this->name() . ':addinstance';
        if ($this->has_add_block_capability($page, $capability)
                && has_capability('lion/block:edit', $page->context)) {
            return true;
        }
    ...
    static function get_extra_capabilities() {
        return array('lion/block:view', 'lion/block:edit');
    }
