YUI.add('lion-form-shortforms', function (Y, NAME) {

/**
 * Provides the form shortforms class.
 *
 * @module lion-form-shortforms
 */
...
Y.extend(SHORTFORMS, Y.Base, {
    ...
    initializer: function() {
        var form = Y.one('#'+this.get('formid')),
            ...
            idlist;
        if (!form) {
            Y.log('Could not locate the form', 'warn', 'lion-form-shortforms');
            return;
        }
    ...
    set_state: function(fieldset, collapsed) {
        ...
        if (!statuselement) {
            Y.log("M.form.shortforms::switch_state was called on an fieldset without a status field: '" +
                fieldset.get('id') + "'", 'debug', 'lion-form-shortforms');
            return this;
        }
    ...
    update_btns: function() {
        ...
        if (expandbtn) {
            ...
            btn.setHTML(M.util.get_string('expandall', 'lion'));
        } else {
            btn.addClass(CSS.COLLAPSEALL);
            btn.setHTML(M.util.get_string('collapseall', 'lion'));
        }

        return this;
    }
}, {
    NAME: 'lion-form-shortforms',
    ATTRS: ATTRS
});
