    ...
    Y.extend(dnduploader, Y.Base, {
        ...
        initialise_queue: function(files) {
            ...
            for (i=0; i<files.length; i++) {
                if (this.options.maxbytes > 0 && files[i].size > this.options.maxbytes) {
                    ...
                    this.print_msg(M.util.get_string('maxbytesforfile', 'lion', files[i].name), 'error');
        ...
        add_to_upload_queue: function(file, filename, overwrite) {
            ...
            if (this.options.maxfiles >= 0 && this.currentfilecount > this.options.maxfiles) {
                ...
                this.uploadqueue = [];
                this.renamequeue = [];
                this.print_msg(M.util.get_string('maxfilesreached', 'lion', this.options.maxfiles), 'error');
                return false;
            }
            ...
            if (this.options.areamaxbytes > -1) {
                ...
                if (sizereached > this.options.areamaxbytes) {
                    this.uploadqueue = [];
                    this.renamequeue = [];
                    this.print_msg(M.util.get_string('maxareabytesreached', 'lion'), 'error');
                    return false;
                }
            }
        ...
        process_renames: function() {
            ...
            node.one('.fp-dlg-text').setContent(M.util.get_string('fileexists', 'lion', file.name));
