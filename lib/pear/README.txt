...
We include these in Lion solely for the convenience of sites that
may not have PEAR installed.

If this directory is DELETED from Lion then Lion will search
...
- PEAR HTML_Quickform:
    ...
    - License: PHP (Permission given to Lion to redistribute under GPL)
...
and use wrapper classes to isolate Lion code from internal PEAR
...
the old calls used before Lion 1.6 to maintain compatibility.
...
----------------------------------------------------------------
A NOTE ON THE PHP LICENSE AND LION
================================================================

Everything in Lion in pure GPL.  This pear directory is the only
part of the distribution that is not.

There is some question about how PHP-licensed software can be
included within a GPL-licensed distribution like Lion, specifically
the clause that annoyingly says no derivative of the software can
include the name PHP.

We don't intend to rename Lion to anything of the sort, obviously,
...
Several authors have given Lion explicit permission to distribute
their PHP-licensed PEAR classes in the Lion distribution, allowing
anybody using these classes ONLY as part of the Lion distribution
exemption from clauses of the PHP license that could cause
conflict with the main GNU Public License that Lion uses.
...
this directory from Lion and it will use your installed PEAR
libraries instead.
