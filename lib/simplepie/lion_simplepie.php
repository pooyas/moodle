 ...
 * 
 *
 * Customised version of SimplePie for Lion
 */
...
/**
 * Lion Customised version of the SimplePie class
 *
 * This class extends the stock SimplePie class
 * in order to make sensible configuration choices,
 * such as using the Lion cache directory and
 * curl functions/proxy config for making http
 * requests in line with lion configuration.
 *
 */
class lion_simplepie extends SimplePie {
    /**
    ...
    public function __construct($feedurl = null, $timeout = 2) {
        $cachedir = lion_simplepie::get_cache_directory();
        check_dir_exists($cachedir);

        parent::__construct();

        // Use the Lion class for http requests
        $this->set_file_class('lion_simplepie_file');

        // Use html purifier for text cleaning.
        $this->set_sanitize_class('lion_simplepie_sanitize');
        $this->sanitize = new lion_simplepie_sanitize();
...
/**
 * Lion Customised version of the SimplePie_File class
 *
 * This class extends the stock SimplePie class
 * in order to utilise Lions own curl class for making
 * http requests. By using the lion curl class
 * we ensure that the correct proxy configuration is used.
 */
class lion_simplepie_file extends SimplePie_File {

    /**
     * The constructor is a copy of the stock simplepie File class which has
     * been modified to add in use the Lion curl class rather than php curl
     * functions.
     */
 ...
 */
class lion_simplepie_sanitize extends SimplePie_Sanitize {
