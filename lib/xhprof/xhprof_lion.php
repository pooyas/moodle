...
function profiling_stop() {
    ...
    // config.php before Lion is installed. Rare but...
    $tables = $DB->get_tables();
    if (!in_array('profiling', $tables)) {
        return false;
    }

    $run = new lion_xhprofrun();
...
/**
 * Export profiling runs to a .mpr (lion profile runs) file.
 *
 ...
 *     - lion_profiling_runs.xml: Metadata about the information
 *         exported. Contains some header information (version and
 *         release of lion, database, git hash - if available, date
 ...
 * profiling table for each run (lion version, database, git hash...).
 *
...
/**
 * Import a .mpr (lion profile runs) file into lion.
 *
...
function profiling_import_runs($file, $commentprefix = '') {
    ...
    $fp = get_file_packer('application/vnd.lion.profiling');
    $status = $fp->extract_to_pathname($file, $tmpdir);

    // Look for master file and verify its format.
    if ($status) {
        $mfile = $tmpdir . '/lion_profiling_runs.xml';
...
function profiling_export_generate(array $runids, $tmpdir) {
    ...
    // Calculate the header information to be sent to lion_profiling_runs.xml.
    $release = $CFG->release;
    $version = $CFG->version;
    $dbtype = $CFG->dbtype;
    ...
    $mainxo = new file_xml_output($tmpdir . '/lion_profiling_runs.xml');
    $mainxw = new xml_writer($mainxo);

    // Output begins.
    $mainxw->start();
    $mainxw->begin_tag('lion_profiling_runs');
    ...
    foreach ($runids as $runid) {
        ...
        $runxw->start();
        $runxw->begin_tag('lion_profiling_run');
        ...
        $runxw->end_tag('lion_profiling_run');
        $runxw->stop();
    }
    $mainxw->end_tag('runs');
    $mainxw->end_tag('lion_profiling_runs');
    $mainxw->stop();
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema" elementFormDefault="qualified">
  <xs:element name="lion_profiling_runs">
    <xs:complexType>
      <xs:sequence>
        <xs:element ref="info"/>
        <xs:element ref="runs"/>
...
function profiling_get_import_run_schema() {
    $schema = <<<EOS
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema" elementFormDefault="qualified">
  <xs:element name="lion_profiling_run">
    <xs:complexType>
      <xs:sequence>
 ...
 */
class lion_xhprofrun implements iXHProfRuns {
...
class xhprof_table_sql extends table_sql {
    ...
    protected function col_url($row) {
        ...
        $scripturl = new lion_url('/admin/tool/profiling/index.php', array('script' => $row->url, 'listurl' => $row->url));
        ...
        if ($this->listurlmode) {
            ...
            $detailsurl = new lion_url('/admin/tool/profiling/index.php', array('listurl' => $row->url));
    ...
    protected function col_timecreated($row) {
        ...
        $url = new lion_url('/admin/tool/profiling/index.php', array('runid' => $row->runid, 'listurl' => $row->url));
