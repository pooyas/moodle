...
!function () {
    ...
    function b(r, H, u) {
        ...
        function C() {
            r.innerHTML=''; // Lion hack - we do not want splashscreens, unfortunately there is not switch to disable them
            if ($f(r)) {
    ...
    window.flowplayer = window.$f = function () {
        var q = null;
        var p = arguments[0];
        if(!flashembed.isSupported([6, 65])){return null;} // Lion hack - we do not want the missing flash hints - we need the original links for accessibility and incompatible browsers
        if (!arguments.length) {
            n(a, function () {
                if (this.isLoaded()) {
                    q = this;
