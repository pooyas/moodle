YUI.add('lion-atto_image-button', function (Y, NAME) {
...
/**
 * @module lion-atto_image_alignment-button
 */
...
Y.namespace('M.atto_image').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _handleDragDrop: function(e) {
        ...
        if (handlesDataTransfer && /^image\//.test(e.dataTransfer.files[0].type)) {
            ...
            uploadid = 'lionimage_' + Math.round(Math.random() * 100000) + '-' + timestamp;
            host.focus();
            host.restoreSelection();
            imagehtml = template({
                url: M.util.image_url("i/loading_small", 'lion'),
                ...
                if (xhr.readyState === 4) {
                    if (xhr.status === 200) {
                        ...
                        Y.use('lion-core-notification-alert', function() {
                            new M.core.alert({message: M.util.get_string('servererror', 'lion')});
                        });
                        if (placeholder) {
                            placeholder.remove(true);
                        }
