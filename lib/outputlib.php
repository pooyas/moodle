...
/**
 * This class represents the configuration variables of a Lion theme.
 *
 ...
 * workings of Lion's themes system. If you are just editing a themes config.php
...
class theme_config {
     ...
     * Pages in Lion are using several different layouts like 'normal', 'course', 'home',
     * 'popup', 'form', .... The most reliable way to get a complete list is to look at
     * {@link http://cvs.lion.org/lion/theme/base/config.php?view=markup the base theme config.php file}.
     ...
     *
     * This is an advanced feature. Lion output is generated by 'renderers',
     * you can customise the HTML that is output by writing custom renderers,
     * and then you need to specify 'renderer factory' so that Lion can find
     * your renderers.
     *
     * There are some renderer factories supplied with Lion. Please follow these
     ...
     *  - 'html5' the only officially supported doctype in Lion
    ...
    public static function load($themename) {
        ...
        try {
            ...
            // most probably lion tables not created yet
        ...
        if (is_readable($rendererfile)) {
            ...
            if (is_readable($this->dir.'/renderer.php')) {
                debugging('Developer hint: '.$this->dir.'/renderer.php should be renamed to ' . $this->dir."/renderers.php.
                    See: http://docs.lion.org/dev/Output_renderers#Theme_renderers.", DEBUG_DEVELOPER);
            }
        }
     ...
     */
    public function init_page(lion_page $page) {
    ...
    public function editor_css_url($encoded=true) {
        ...
        if ($rev > -1) {
            $url = new lion_url("$CFG->httpswwwroot/theme/styles.php");
            ...
            $url = new lion_url($CFG->httpswwwroot.'/theme/styles_debug.php', $params);
        }
        return $url;
    }
     ...
     *
     * @param lion_page $page Not used... deprecated?
     * @return lion_url[]
     */
    public function css_urls(lion_page $page) {
        global $CFG;
        ...
        if ($rev > -1) {
            ...
            $baseurl = new lion_url($CFG->httpswwwroot.'/theme/styles_debug.php');
            ...
            if (core_useragent::is_ie()) {
                ...
                $urls[] = new lion_url($baseurl, array('theme'=>$this->name, 'type'=>'ie', 'subtype'=>'plugins'));
                foreach ($css['parents'] as $parent=>$sheets) {
                    ...
                    $urls[] = new lion_url($baseurl, array('theme'=>$this->name,'type'=>'ie', 'subtype'=>'parents', 'sheet'=>$parent));
                }
                if (!empty($this->lessfile)) {
                    // No need to define the type as IE here.
                    $urls[] = new lion_url($baseurl, array('theme' => $this->name, 'type' => 'less'));
                }
                $urls[] = new lion_url($baseurl, array('theme'=>$this->name, 'type'=>'ie', 'subtype'=>'theme'));

            } else {
                foreach ($css['plugins'] as $plugin=>$unused) {
                    $urls[] = new lion_url($baseurl, array('theme'=>$this->name,'type'=>'plugin', 'subtype'=>$plugin));
                }
                foreach ($css['parents'] as $parent=>$sheets) {
                    foreach ($sheets as $sheet=>$unused2) {
                        $urls[] = new lion_url($baseurl, array('theme'=>$this->name,'type'=>'parent', 'subtype'=>$parent, 'sheet'=>$sheet));
                    }
                }
                foreach ($css['theme'] as $sheet => $filename) {
                    if ($sheet === $this->lessfile) {
                        ...
                        $urls[] = new lion_url($baseurl, array('sheet'=>$sheet, 'theme'=>$this->name, 'type'=>'theme'));
                    }
                }
            }
        }
    ...
    public function get_css_content() {
        ...
        if (!empty($CFG->enablecssoptimiser) && $this->supportscssoptimisation) {
            // This is an experimental feature introduced in Lion 2.3
    ...
    public function javascript_url($inhead) {
        ...
        if (!empty($CFG->slasharguments) and $rev > 0) {
            $url = new lion_url("$CFG->httpswwwroot/theme/javascript.php");
            $url->set_slashargument('/'.$this->name.'/'.$rev.'/'.$params['type'], 'noparam', true);
            return $url;
        } else {
            return new lion_url($CFG->httpswwwroot.'/theme/javascript.php', $params);
        }
    }
    ...
    public function pix_url($imagename, $component) {
        ...
        if (empty($component) or $component === 'lion' or $component === 'core') {
        ...
        $url = new lion_url("$CFG->httpswwwroot/theme/image.php");
        if (!empty($CFG->slasharguments) and $rev > 0) {
    ...
    public function font_url($font, $component) {
        ...
        $url = new lion_url("$CFG->httpswwwroot/theme/font.php");
        if (!empty($CFG->slasharguments) and $rev > 0) {
    ...
    public function setting_file_url($setting, $filearea) {
        ...
        $url = lion_url::make_file_url("$CFG->wwwroot/pluginfile.php", "/$syscontext->id/$component/$filearea/$itemid".$filepath);

        // Now this is tricky because the we can not hardcode http or https here, lets use the relative link.
        // Note: unfortunately lion_url does not support //urls yet.
    ...
    public function resolve_image_location($image, $component, $svg = false) {
        ...
        if ($component === 'lion' or $component === 'core' or empty($component)) {
     ...
     * At that point the were the following image counts in Lion core:
     *
    ...
    /**
     * Get the renderer for a part of Lion for this theme.
     *
     * @param lion_page $page the page we are rendering
     * @param string $component the name of part of lion. E.g. 'core', 'quiz', 'qtype_multichoice'.
