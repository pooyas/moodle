...
/**
 * lionlib.php - Lion main library
 *
 * Main library file of miscellaneous general-purpose Lion functions.
...
/**
 * PARAM_CAPABILITY - A capability name, like 'lion/role:manage'. Actually
...
/**
 * PAGE_COURSE_VIEW is a definition of a page type. For more information on the page class see lion/lib/pagelib.php.
 */
...
/** True if module supports backup/restore of lion2 format */
define('FEATURE_BACKUP_LION2', 'backup_lion2');
...
/**
 * Hub directory url (should be lion.org)
 */
define('HUB_HUBDIRECTORYURL', "http://hubdirectory.lion.org");


/**
 * Lion.org url (should be lion.org)
 */
define('HUB_LIONORGHUBURL', "http://hub.lion.org");

/**
 * Lion mobile app service name
 */
define('LION_OFFICIAL_MOBILE_SERVICE', 'lion_mobile_app');
...
function required_param($parname, $type) {
    ...
    if (is_array($param)) {
        ...
        // TODO: switch to fatal error in Lion 2.3.
...
function optional_param($parname, $default, $type) {
    ...
    if (is_array($param)) {
        debugging('Invalid array parameter detected in required_param(): '.$parname);
        // TODO: switch to $default in Lion 2.3.
...
function get_config($plugin, $name = null) {
    ...
    if ($plugin === 'lion' || $plugin === 'core' || empty($plugin)) {
 ...
 * @param    stdClass|int|null $user  A lion user object or id, null means current user
 * @throws   coding_exception
 ...
 *             if 99 then default user's timezone is used {@link http://docs.lion.org/dev/Time_API#Timezone}
 ...
 *        dst offset is applied {@link http://docs.lion.org/dev/Time_API#Timezone}
 ...
 *        then user's default timezone is used. {@link http://docs.lion.org/dev/Time_API#Timezone}
 * @return int
 */
...
function require_login($courseorid = null, $autologinguest = true, $cm = null, $setwantsurltome = true, $preventredirect = false) {
    ...
    if (!empty($CFG->maintenance_enabled) and !has_capability('lion/site:config', $sysctx)) {
        if ($preventredirect) {
    ...
    if ($course->id == SITEID) {
        ...
        if (is_role_switched($course->id)) {
            ...
            if (!$course->visible and !has_capability('lion/course:viewhiddencourses', $coursecontext)) {
                ...
                navigation_node::override_active_url(new lion_url('/'));
 ...
 *
 * @throws lion_exception If a problem occurs while generating the hash.
 */
function hash_internal_user_password($password, $fasthash = false) {
    ...
    if ($generatedhash === false || $generatedhash === null) {
        throw new lion_exception('Failed to generate password hash.');
    }
 ...
 */
function lion_process_email($modargs, $body) {
    global $DB;
 ...
 * @return lion_phpmailer|null mailer instance if 'get' used or nothing
 */
function get_mailer($action='get') {
    global $CFG;

    /** @var lion_phpmailer $mailer */
    static $mailer  = null;
    static $counter = 0;
    ...
    if ($action == 'get') {
        ...
        require_once($CFG->libdir.'/phpmailer/lion_phpmailer.php');
        $mailer = new lion_phpmailer();
...
function email_to_user($user, $from, $subject, $messagetext, $messagehtml = '', $attachment = '', $attachname = '',
    ...
    if ($mail->send()) {
        ...
        $event->trigger();
        if (CLI_SCRIPT) {
            mtrace('Error: lib/lionlib.php email_to_user(): '.$mail->ErrorInfo);
        }
        if (!empty($mail->SMTPDebug)) {
            echo '</pre>';
        }
...
function send_password_change_info($user) {
    ...
    if (!empty($data->link) and has_capability('lion/user:changeownpassword', $systemcontext, $user->id)) {
...
function get_file_packer($mimetype='application/zip') {
    ...
    switch ($mimetype) {
        ...
        case 'application/vnd.lion.profiling':
        ...
        case 'application/vnd.lion.backup':
 ...
 * 5. by the Lion admin in $CFG->maxbytes
...
function get_user_max_upload_file_size($context, $sitebytes = 0, $coursebytes = 0, $modulebytes = 0, $user = null) {
    ...
    if (has_capability('lion/course:ignorefilesizelimits', $context, $user)) {
 ...
 * to retrieve it.<br/>
 * If you open lion/lang/en/lion.php and look near line 278
 ...
 * different approach. Looking in lion/lang/en/calendar.php you find
 * around line 75:
 * <code>
 ...
 *
 * In Lion 2.3 there is a new argument to this function $lazyload.
 ...
 *      If none is specified then lion.php is used.
 ...
 *
 * Example usage of this function when the string is in the lion.php file:<br/>
 * <code>
 * echo '<strong>';
 * print_string('course');
 * echo '</strong>';
 * </code>
 *
 * Example usage of this function when the string is not in the lion.php file:<br/>
 * <code>
 * echo '<h1>';
 ...
 * @category string
 * @param string $identifier The key identifier for the localized string
 * @param string $component The module where the key identifier is stored. If none is specified then lion.php is used.
...
class emoticon_manager {
    ...
    /**
     * Returns default set of emoticons supported by Lion
     *
 ...
 *
 * This function was originally used for standard Lion plugins, please use
...
/**
 * Determine if lion installation requires update.
 *
 ...
 *
 * Lion version numbers consist of three numbers separated by a dot, for
 ...
 */
function lion_major_version($fromdisk = false) {
    global $CFG;
 ...
 */
function lion_setlocale($locale='') {
    global $CFG;
 ...
 * replace an existing lion script
 *
 ...
 * is in lionlib because it does not rely on loading any of the MNET code.
 *
 * @param object $user A valid user object
 * @return bool        True if the user is from a remote Lion.
 */
...
/**
 * This function will search for browser prefereed languages, setting Lion
...
function set_mnet_remote_client($client) {
    if (!defined('MNET_SERVER')) {
        throw new lion_exception('notinxmlrpcserver', 'mnet');
    }
    global $MNET_REMOTE_CLIENT;
 ...
 * @param stdClass|course_in_list $course Lion course object
 ...
 *     $string = get_string('yes', 'lion', null, true);
