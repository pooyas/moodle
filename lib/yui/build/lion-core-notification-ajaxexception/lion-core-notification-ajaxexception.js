YUI.add('lion-core-notification-ajaxexception', function (Y, NAME) {
 ...
 * thrown in the Lion codebase was reported during an AJAX request.
 *
 * @module lion-core-notification
 * @submodule lion-core-notification-ajaxexception
 */

var AJAXEXCEPTION_NAME = 'Lion AJAX exception',
    AJAXEXCEPTION;
...
Y.extend(AJAXEXCEPTION, M.core.notification.info, {
    ...
    initializer : function(config) {
        ...
        this.get(BASE).addClass('lion-dialogue-exception');
        this.setStdModContent(Y.WidgetStdMod.HEADER,
                '<h1 id="lion-dialogue-'+this.get('COUNT')+'-header-text">' + Y.Escape.html(config.name) + '</h1>',
                Y.WidgetStdMod.REPLACE);
        content = Y.Node.create('<div class="lion-ajaxexception"></div>')
                .append(Y.Node.create('<div class="lion-exception-message">'+Y.Escape.html(this.get('error'))+'</div>'))
                .append(Y.Node.create('<div class="lion-exception-param hidden param-debuginfo"><label>URL:</label> ' +
                        this.get('reproductionlink')+'</div>'))
                .append(Y.Node.create('<div class="lion-exception-param hidden param-debuginfo"><label>Debug info:</label> ' +
                        Y.Escape.html(this.get('debuginfo'))+'</div>'))
                .append(Y.Node.create('<div class="lion-exception-param hidden param-stacktrace">' +
                                      '<label>Stack trace:</label> <pre>' +
                        Y.Escape.html(this.get('stacktrace'))+'</pre></div>'));
        if (M.cfg.developerdebug) {
            content.all('.lion-exception-param').removeClass('hidden');
        }
    ...
    ATTRS : {
        ...
        error : {
            ...
            value: M.util.get_string('unknownerror', 'lion')
        },
...
}, '@VERSION@', {"requires": ["lion-core-notification-dialogue"]});
