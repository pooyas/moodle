...
/**
 * Native mysqli class representing lion database interface.
 *
 * @package    core
 * @subpackage dml
 ...
 *
 */
class mysqli_native_lion_database extends lion_database {
     ...
     * Lion is designed to work best on ACID compliant databases
    ...
    public function get_dbengine() {
        ...
        if ($engine === 'MyISAM') {
            // we really do not want MyISAM for Lion, InnoDB or XtraDB is a reasonable defaults if supported
    ...
    public function connect($dbhost, $dbuser, $dbpass, $dbname, $prefix, array $dboptions=null) {
        ...
        // With strict mode enforced, Lion DB layer will be consistently throwing
        ...
        $this->temptables = new mysqli_native_lion_temptables($this);
    ...
    /**
     * Returns lion column info for raw column from information schema.
    ...
    private function get_column_info(stdClass $rawcolumn) {
        ...
        $info->meta_type      = $this->mysqltype2liontype($rawcolumn->data_type);
        ...
        if ($info->meta_type === 'C') {
            ...
            if ("$rawcolumn->character_maximum_length" === '4294967295') { // watch out for PHP max int limits!
                // means maximum lion size for text column, in other drivers it may also mean unknown size
                $info->max_length = -1;
            } else {
                $info->max_length = $rawcolumn->character_maximum_length;
            }
     ...
     */
    private function mysqltype2liontype($mysql_type) {
        $type = null;
    ...
    protected function create_recordset($result) {
        return new mysqli_native_lion_recordset($result);
    }
