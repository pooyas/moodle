 ...
 * cross-db compatibility in the Lion 2.x DB API with some operations
 * not natively supported by Oracle, namely:
 *  - locking: Application locks used by Lion DB sessions. It uses
 *             the DBMS_LOCK package so execution must be granted
 *             to the Lion DB user by SYS to work properly.
...
CREATE OR REPLACE PACKAGE LIONLIB AS
...
CREATE OR REPLACE PACKAGE BODY LIONLIB AS
...
BEGIN
    RETURN LIONLIB.BITOR(value1,value2) - BITAND(value1,value2);
END BITXOR;
...
BEGIN
    ...
    stringresult := CONCAT(CONCAT(LIONLIB.UNDO_DIRTY_HACK(string1), LIONLIB.UNDO_DIRTY_HACK(string2)), LIONLIB.UNDO_DIRTY_HACK(string3));
