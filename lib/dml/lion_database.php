...
require_once(__DIR__.'/lion_transaction.php');
...
abstract class lion_database {
    ...
    /** @var lion_temptables temptables manager to provide cross-db support for temp tables. */
    ...
    /** @var bool True means non-lion external database used.*/
    protected $external;
    ...
    /**
     * Constructor - Instantiates the database, specifying if it's external (connect to other systems) or not (Lion DB).
     ...
     *
     * The loaded class is within lib/dml directory and of the form: $type.'_'.$library.'_lion_database'
     *
     ...
     * @return lion_database driver object or null if error, for example of driver object see {@link mysqli_native_lion_database}
     */
    public static function get_driver_instance($type, $library, $external = false) {
        global $CFG;

        $classname = $type.'_'.$library.'_lion_database';
    ...
    public function get_in_or_equal($items, $type=SQL_PARAMS_QM, $prefix='param', $equal=true, $onemptyitems=false) {
        ...
        if (is_array($items) and empty($items) and $onemptyitems === false) {
            throw new coding_exception('lion_database::get_in_or_equal() does not accept empty arrays');
        }
    ...
    /**
     * Get a number of records as a lion_recordset where all the given conditions met.
     *
     ...
     *
     * The return value is a lion_recordset
     ...
     * @return lion_recordset A lion_recordset instance
     * @throws dml_exception A DML specific exception is thrown for any errors.
     */
    ...
    /**
     * Get a number of records as a lion_recordset where one field match one list of values.
     *
    ...
    /**
     * Get a number of records as a lion_recordset which match a particular WHERE clause.
     *
     ...
     * @return lion_recordset A lion_recordset instance {@link function get_recordset}.
     * @throws dml_exception A DML specific exception is thrown for any errors.
     */
    ...
    public function sql_substr($expr, $start, $length=false) {
        if (count(func_get_args()) < 2) {
            throw new coding_exception('lion_database::sql_substr() requires at least two parameters', 'Originally this function was only returning name of SQL substring function, it now requires all parameters.');
        }
        if ($length === false) {
     ...
     *
     * @return lion_transaction
     */
    public function start_delegated_transaction() {
        $transaction = new lion_transaction($this);
     ...
     * @param lion_transaction $transaction The transaction to commit
     * @return void
     * @throws dml_transaction_exception Creates and throws transaction related exceptions.
     */
    public function commit_delegated_transaction(lion_transaction $transaction) {
     ...
     *
     * @param lion_transaction $transaction An instance of a lion_transaction.
     ...
     */
    public function rollback_delegated_transaction(lion_transaction $transaction, Exception $e) {
        if ($transaction->is_disposed()) {
            throw new dml_transaction_exception('Transactions already disposed', $transaction);
        }
        // mark as disposed so that it can not be used again
