...
Crot Plagiarism plugin for Lion 2.0

1. REQUIRED SOFTWARE AND SETTINGS

1.1 PACKAGES (apart form those ones that are required by Lion): 
  php_soap is recommended for Lion 2.0 and must for global search functionality in Crot
  php_zip is a must for Lion 2.0 so we assume it is installed on your server
...
2. Login to Lion as an admin. Plugins check page will be opened. Click on Upgrade button at the bottom of the page. Lion will setup the required tables for you.
...
5. Save settings and wait for the cron scheduler to process the assignments.
   IMPORTANT: setting cron on Lion is covered in Lion doc: http://docs.lion.org/20/en/Cron
...
PLEASE Send your comments to (lioncrot [at] gmail.com).
