...
///COMMENTOUT(Pooya)
///Preventing the giving of the notice
        //$file = $linkarray['file'];
///CODEADDED(Pooya)
///Compensating the above commentout
        if(!empty($linkarray['file'])) $file = $linkarray['file']; else $file=array();
        $course = $linkarray['course'];
///CODEADDED(Pooya)
///Preventing the php notice for undifined variable
        $output = '';
//sw 08/27
///COMMENTOUT(Pooya)
...
//        if (!empty($plagiarismsettings['crot_use'])) { //sw 4-21
 //           if (isset($data->crot_use)) { //sw 4-21
		...
		$fileid=null;
              ...
              if (!$fileid && !$plagiarism_crot_files_rec = $DB->get_record("plagiarism_crot_files", array("file_id"=>$fileid))) {
///COMMENTOUT(Pooya)
///There is a small pending under the forum posts which this will remove;
...
//} // sw 4-41
    ...
    */
     ...
     */
///COMMENTOUT(Pooya)
///The definition doesn't match the one from the plagiarism api so it gives warnings
//    public function get_form_elements_module($mform, $context) {
///CODEADDED(Pooya)
///Replacing the above commented code
    ...
    public function print_disclosure($cmid) {
        ...
        echo format_text($plagiarismsettings['crot_student_disclosure'], FORMAT_LION, $formatoptions);
        echo $OUTPUT->box_end();
    }
...
function crot_event_content_uploaded($eventdata) {
    ...
    if ($eventdata->modulename == "assign") {
      ...
      $filepath = "/"; // has to start and end with /, rule of the Lion file storage api
      print_r($eventdata);
      ...
      $fs = get_file_storage(); // using file storage system ensures that the file can be located by other api calls
                                // this creates a hash-version of the file in the Lion data folder and also creates entries in the 'files' table
