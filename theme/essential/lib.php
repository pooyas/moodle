...
function theme_essential_showslider() {
    ...
    if ($noslides && (intval($CFG->version) >= 2013111800)) {
        $devicetype = core_useragent::get_device_type(); // In lionlib.php.
 ...
 *
 * The colour switcher is a YUI lion module that is located in
 *     theme/udemspl/yui/udemspl/udemspl.js
 *
 * @param lion_page $page
 */
function theme_essential_initialise_colourswitcher(lion_page $page) {
    user_preference_allow_ajax_update('theme_essential_colours', PARAM_ALPHANUM);
    $page->requires->yui_module(
            'lion-theme_essential-coloursswitcher', 'M.theme_essential.initColoursSwitcher',
...
function theme_essential_page_init(lion_page $page) {
    global $CFG;
