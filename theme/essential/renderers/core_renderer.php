...
class theme_essential_core_renderer extends core_renderer {
    ...
    public function custom_menu_language() {
        ...
        if ($addlangmenu) {
            ...
            $this->language = $langmenu->add('<i class="fa fa-flag"></i>' . $currentlang, new lion_url('#'), $strlang, 100);
            foreach ($langs as $langtype => $langname) {
                $this->language->add('<i class="fa fa-language"></i>' . $langname, new lion_url($this->page->url, array('lang' => $langtype)), $langname);
            }
        }
    ...
    public function custom_menu_courses() {
        ...
        if (isloggedin() && !isguestuser() && $hasdisplaymycourses) {
            ...
            $branch->add($homelabel, new lion_url('/my/index.php'), $hometext);
            ...
            if ($courses = enrol_get_my_courses(NULL, $sortorder . ' ASC')) {
                foreach ($courses as $course) {
                    if ($course->visible) {
                        $branch->add('<i class="fa fa-graduation-cap"></i>' . format_string($course->fullname), new lion_url('/course/view.php?id=' . $course->id), format_string($course->shortname));
                        $numcourses += 1;
                    } else if (has_capability('lion/course:viewhiddencourses', context_system::instance())) {
                        ...
                        $branchurl = new lion_url('/course/view.php', array('id' =>$course->id));
            ...
            if ($numcourses == 0 || empty($courses)) {
                ...
                $branch->add('<em>' . $noenrolments . '</em>', new lion_url('#'), $noenrolments);
            }
        }
        ...
        if (!isguestuser()) {
            ...
            if (!empty($alternativethemes)) {
                ...
                $branch->add('<i class="fa fa-square colours-default"></i>' . $defaultthemecolorslabel,
                    new lion_url($this->page->url, array('essentialcolours' => 'default')), $defaultthemecolorslabel);
                foreach ($alternativethemes as $alternativethemenumber) {
                    ...
                    $branch->add('<i class="fa fa-square colours-alternative' . $alternativethemenumber . '"></i>' . $alternativethemeslabel,
                        new lion_url($this->page->url, array('essentialcolours' => 'alternative' . $alternativethemenumber)), $alternativethemeslabel);
                }
            }
        }
    ...
    public function custom_menu_activitystream() {
        ...
        if (!isguestuser()) {
            if (isset($this->page->course->id) && $this->page->course->id > 1) {
                ...
                $branchurl = new lion_url('/user/index.php', array('id' => $this->page->course->id));
                ...
                $branchurl = new lion_url('/grade/report/index.php', array('id' => $this->page->course->id));
                ...
                foreach ($data as $modname => $modfullname) {
                    if ($modname === 'resources') {
                        ...
                        $branch->add($icon.$modfullname, new lion_url('/course/resources.php', array('id' => $this->page->course->id)));
                    } else {
                        $icon = '<img src="'.$this->pix_url('icon', $modname) . '" class="icon" alt="" />';
                        $branch->add($icon.$modfullname, new lion_url('/mod/'.$modname.'/index.php', array('id' => $this->page->course->id)));
                    }
                }
    ...
    public function custom_menu_messages() {
        ...
        if (empty($totalmessages)) {
            ...
            $messagemenu->add(
                $messagemenutext,
                new lion_url('/message/index.php', array('viewing' => 'recentconversations')),
                $messagetitle,
                9999
            );
        } else {
    ...
    private function process_message($message) {
        ...
        if ($message->notification || $message->useridfrom < 1) {
            ...
            $messagecontent->url = new lion_url($message->contexturl);
            if (empty($message->contexturl)) {
                $messagecontent->url = new lion_url('/message/index.php', array('user1' => $USER->id, 'viewing' => 'recentnotifications'));
            }
        } else {
            ...
            $messagecontent->url = new lion_url('/message/index.php', array('user1' => $USER->id, 'user2' => $message->useridfrom));
        }
    ...
    public function custom_menu_goto_bottom() {
        ...
        if (($this->page->pagelayout == 'course') || ($this->page->pagelayout == 'incourse') || ($this->page->pagelayout == 'admin')) { // Go to bottom.
            ...
            $menu->add($gotobottom, new lion_url('#region-main'), get_string('gotobottom', 'theme_essential'));
    ...
    public function custom_menu_user() {
        ...
        if (!isloggedin()) {
            ...
            $branchurl = new lion_url('/login/logout.php?sesskey=' . sesskey());
            ...
            if (\core\session\manager::is_loggedinas()) {
                ...
                $branchurl = new lion_url('/user/profile.php', array('id' => $USER->id));
                $usermenu .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
            } else {
            ...
            if (is_mnet_remote_user($USER) && $idprovider = $DB->get_record('mnet_host', array('id' => $USER->mnethostid))) {
                ...
                $branchurl = new lion_url($idprovider->wwwroot);
                $usermenu .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
            }

            if (is_role_switched($course->id)) { // Has switched roles
                $branchlabel = '<em><i class="fa fa-users"></i>' . get_string('switchrolereturn') . '</em>';
                $branchurl = new lion_url('/course/switchrole.php', array('id' => $course->id, 'sesskey' => sesskey(), 'switchrole' => 0, 'returnurl' => $this->page->url->out_as_local_url(false)));
                $usermenu .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
            }
            ...
            if (has_capability('lion/calendar:manageownentries', $context)) {
                $branchlabel = '<em><i class="fa fa-calendar"></i>' . get_string('pluginname', 'block_calendar_month') . '</em>';
                $branchurl = new lion_url('/calendar/view.php');
                $usermenu .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
            }
            ...
            if (!empty($CFG->messaging)) {
                ...
                $branchurl = new lion_url('/message/index.php');
                $usermenu .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
            }
            ...
            if (has_capability('lion/user:manageownfiles', $context)) {
                ...
                $branchurl = new lion_url('/user/files.php');
                $usermenu .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
            }
            ...
            if (has_capability('mod/forum:viewdiscussion', $context)) {
                ...
                $branchurl = new lion_url('/mod/forum/user.php', array('id' => $USER->id));
                $usermenu .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));

                $branchlabel = '<em><i class="fa fa-list"></i>' . get_string('discussions', 'mod_forum') . '</em>';
                $branchurl = new lion_url('/mod/forum/user.php', array('id' => $USER->id, 'mode' => 'discussions'));
                $usermenu .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
            ...
            if ($course->id == 1) {
                ...
                foreach ($hascourses as $hascourse) {
                    ...
                    if (has_capability('gradereport/user:view', $reportcontext) && $hascourse->visible) {
                        ...
                        $branchurl = new lion_url('/grade/report/overview/index.php' , array('id' => $hascourse->id, 'userid' => $USER->id));
                        $usermenu .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
                    }
                }
            } else if (has_capability('gradereport/user:view', $context)) {
                $branchlabel = '<em><i class="fa fa-list-alt"></i>' . get_string('mygrades', 'theme_essential') . '</em>';
                $branchurl = new lion_url('/grade/report/overview/index.php' , array('id' => $course->id, 'userid' => $USER->id));
                $usermenu .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
                ...
                $branchurl = new lion_url('/grade/report/user/index.php' , array('id' => $course->id, 'userid' => $USER->id));
                $usermenu .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
            }

            // Check if badges are enabled.
            if (!empty($CFG->enablebadges) && has_capability('lion/badges:manageownbadges', $context)) {
                $branchlabel = '<em><i class="fa fa-certificate"></i>' . get_string('badges') . '</em>';
                $branchurl = new lion_url('/badges/mybadges.php');
                $usermenu .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
            }
            $usermenu .= html_writer::empty_tag('hr', array('class' => 'sep'));
    ...
    protected function theme_essential_render_preferences($context) {
        ...
        $preferences .= html_writer::link(new lion_url('#'), $label, array('class' => 'dropdown-toggle', 'data-toggle' => 'dropdown'));
        $preferences .= html_writer::start_tag('ul', array('class' => 'dropdown-menu'));
        // Check if user is allowed to edit profile
        if (has_capability('lion/user:editownprofile', $context)) {
            $branchlabel = '<em><i class="fa fa-user"></i>' . get_string('editmyprofile') . '</em>';
            $branchurl = new lion_url('/user/edit.php', array('id' => $USER->id));
            $preferences .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
        }
        if (has_capability('lion/user:changeownpassword', $context)) {
            $branchlabel = '<em><i class="fa fa-key"></i>' . get_string('changepassword') . '</em>';
            $branchurl = new lion_url('/login/change_password.php');
            $preferences .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
        }
        if (has_capability('lion/user:editownmessageprofile', $context)) {
            $branchlabel = '<em><i class="fa fa-comments"></i>' . get_string('messagepreferences', 'theme_essential') . '</em>';
            $branchurl = new lion_url('/message/edit.php', array('id' => $USER->id));
            $preferences .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
        }
        if ($CFG->enableblogs) {
            $branchlabel = '<em><i class="fa fa-rss-square"></i>' . get_string('blogpreferences', 'theme_essential') . '</em>';
            $branchurl = new lion_url('/blog/preferences.php');
            $preferences .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
        }
        if ($CFG->enablebadges && has_capability('lion/badges:manageownbadges', $context)) {
            $branchlabel = '<em><i class="fa fa-certificate"></i>' . get_string('badgepreferences', 'theme_essential') . '</em>';
            $branchurl = new lion_url('/badges/preferences.php');
            $preferences .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
        }
    ...
    */
    ...
    public function render_pix_icon(pix_icon $icon) {
        if (self::replace_lion_icon($icon->pix)) {
            $newicon = self::replace_lion_icon($icon->pix, $icon->attributes['alt']) . parent::render_pix_icon($icon) . "</i>";
            return $newicon;
        } else {
    ...
    private static function replace_lion_icon($icon, $alt = false) {
        $icons = array(
            'add' => 'plus',
            'book' => 'book',
            'chapter' => 'file',
     ...
     *  in lionlib.php. Returns a string
