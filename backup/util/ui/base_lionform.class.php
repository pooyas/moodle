...
abstract class base_lionform extends lionform {
        ...
        if ($setting->get_visibility() == backup_setting::VISIBLE) {
            ...
            switch ($setting->get_status()) {
                case backup_setting::LOCKED_BY_PERMISSION:
                    $icon = ' '.$OUTPUT->pix_icon('i/permissionlock', get_string('lockedbypermission', 'backup'), 'lion',
                            array('class' => 'smallicon lockedicon permissionlock'));
                    break;
                case backup_setting::LOCKED_BY_CONFIG:
                    $icon = ' '.$OUTPUT->pix_icon('i/configlock', get_string('lockedbyconfig', 'backup'), 'lion',
                            array('class' => 'smallicon lockedicon configlock'));
                    break;
                case backup_setting::LOCKED_BY_HIERARCHY:
                    $icon = ' '.$OUTPUT->pix_icon('i/hierarchylock', get_string('lockedbyhierarchy', 'backup'), 'lion',
                            array('class' => 'smallicon lockedicon configlock'));
                    break;
                default:
                    $icon = '';
     ...
     *
     * @param string $elementname
     * @return HTML_QuickForm_input|LionQuickForm_group
     */
    ...
    public function display() {
        ...
        $PAGE->requires->yui_module('lion-backup-backupselectall', 'M.core_backup.backupselectall',
                array($modnames));
        $PAGE->requires->strings_for_js(array('select', 'all', 'none'), 'lion');
