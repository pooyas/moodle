...
abstract class cc_convert_lion2 {
    ...
    public static function convert($packagedir, $outdir) {
        ...
        if ($doc->load($coursefile)) {
            ...
            $lion_backup = $dir . DIRECTORY_SEPARATOR . 'lion_backup.xml';
            $secp = new XMLGenericDocument();
            $docp = new XMLGenericDocument();
            if ($docp->load($lion_backup)) {
                //sections
                $sections = array();
                ...
                $section_list = $docp->nodeList('/lion_backup/information/contents/sections/section');
    ...
    */
    protected static function process_sequence(cc_i_item &$item, cc_i_manifest &$manifest, array $sequence, $packageroot, $outdir) {
        $lion_backup = $packageroot . DIRECTORY_SEPARATOR . 'lion_backup.xml';
        $doc = new XMLGenericDocument();
        if(!$doc->load($lion_backup)) {
            return;
        }
        $activities = $doc->nodeList('/lion_backup/information/contents/activities/activity');
        if (!empty($activities)) {
