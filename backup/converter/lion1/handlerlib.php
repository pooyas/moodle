 ...
 */
abstract class lion1_handlers_factory {

    /**
     * @param lion1_converter the converter requesting the converters
     * @return list of all available conversion handlers
     */
    public static function get_handlers(lion1_converter $converter) {

        $handlers = array(
            new lion1_root_handler($converter),
            new lion1_info_handler($converter),
            new lion1_course_header_handler($converter),
            new lion1_course_outline_handler($converter),
            new lion1_roles_definition_handler($converter),
            new lion1_question_bank_handler($converter),
            new lion1_scales_handler($converter),
            new lion1_outcomes_handler($converter),
            new lion1_gradebook_handler($converter),
        );
        ...
        foreach ($handlers as $handler) {
            if (!$handler instanceof lion1_handler) {
                throw new lion1_convert_exception('wrong_handler_class', get_class($handler));
            }
        }
     ...
     * @param lion1_converter $converter the converter requesting the handler
     * @throws lion1_convert_exception
     * @return array of {@link lion1_handler} instances
     */
    protected static function get_plugin_handlers($type, lion1_converter $converter) {
        global $CFG;
        ...
        foreach ($plugins as $name => $dir) {
            ...
            $handlerclass = "lion1_{$type}_{$name}_handler";
            if (file_exists($handlerfile)) {
                require_once($handlerfile);
            } elseif ($type == 'block') {
                $handlerclass = "lion1_block_generic_handler";
            } else {
                continue;
            }
 ...
 */
abstract class lion1_handler implements loggable {

    /** @var lion1_converter */
    protected $converter;

    /**
     * @param lion1_converter $converter the converter that requires us
     */
    public function __construct(lion1_converter $converter) {
        $this->converter = $converter;
    }

    /**
     * @return lion1_converter the converter that required this handler
     */
 ...
 */
abstract class lion1_xml_handler extends lion1_handler {
    ...
    protected function open_xml_writer($filename) {
        ...
        if (!is_null($this->xmlfilename) and $filename !== $this->xmlfilename) {
            throw new lion1_convert_exception('xml_writer_already_opened_for_other_file', $this->xmlfilename);
        }
        ...
        if (!$this->xmlwriter instanceof xml_writer) {
            ...
            if (!check_dir_exists($directory)) {
                throw new lion1_convert_exception('unable_create_target_directory', $directory);
            }
            $this->xmlwriter = new xml_writer(new file_xml_output($fullpath), new lion1_xml_transformer());
    ...
    protected function write_xml($element, array $data, array $attribs = array(), $parent = '/') {
        ...
        if (!$this->has_xml_writer()) {
            throw new lion1_convert_exception('write_xml_without_writer');
        }
 ...
 */
class lion1_root_handler extends lion1_xml_handler {

    public function get_paths() {
        return array(new convert_path('root_element', '/LION_BACKUP'));
    }
    ...
    public function on_root_element_start() {
        ...
        $fileshandler = new lion1_files_handler($this->converter);
        $fileshandler->process();
    }

    /**
     * This is executed at the end of the lion.xml parsing
     */
    public function on_root_element_end() {
        ...
        ////////////////////////////////////////////////////////////////////////
        // write lion_backup.xml
        ////////////////////////////////////////////////////////////////////////
        $this->open_xml_writer('lion_backup.xml');

        $this->xmlwriter->begin_tag('lion_backup');
        $this->xmlwriter->begin_tag('information');

        // lion_backup/information
        $this->xmlwriter->full_tag('name', $backupinfo['name']);
        $this->xmlwriter->full_tag('lion_version', $backupinfo['lion_version']);
        $this->xmlwriter->full_tag('lion_release', $backupinfo['lion_release']);
        ...
        // lion_backup/information/contents
        $this->xmlwriter->begin_tag('contents');

        // lion_backup/information/contents/activities
        ...
        // lion_backup/information/contents/sections
        ...
        foreach ($this->converter->get_stash_itemids('sectioninfo') as $sectionid) {
            ...
            $sectionsettings[] = array(
                ...
                'value'     => 0); // @todo how to detect this from lion.xml?
        ...
        // lion_backup/information/contents/course
        ...
        $this->xmlwriter->end_tag('lion_backup');
        ...
        // missing file may happen if the watched tag is not present in lion.xml (for example
        // QUESTION_CATEGORIES is optional in lion.xml but questions.xml must exist in
        // lion2 format) or the handler has not been implemented yet.
 ...
 */
class lion1_files_handler extends lion1_xml_handler {
...
class lion1_info_handler extends lion1_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path('info', '/LION_BACKUP/INFO'),
            new convert_path('info_details', '/LION_BACKUP/INFO/DETAILS'),
            new convert_path('info_details_mod', '/LION_BACKUP/INFO/DETAILS/MOD'),
            new convert_path('info_details_mod_instance', '/LION_BACKUP/INFO/DETAILS/MOD/INSTANCES/INSTANCE'),
        );
    }

    /**
     * Stashes the backup info for later processing by {@link lion1_root_handler}
     */
    ...
    public function on_info_details_mod_end($data) {
        ...
        // keep only such modules that seem to have the support for lion1 implemented
        $modname = $this->currentmod['name'];
        if (file_exists($CFG->dirroot.'/mod/'.$modname.'/backup/lion1/lib.php')) {
    ...
    /**
     * Stashes the list of activity module types for later processing by {@link lion1_root_handler}
     */
...
/**
 * Handles the conversion of /LION_BACKUP/COURSE/HEADER paths
 */
class lion1_course_header_handler extends lion1_xml_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path(
                'course_header', '/LION_BACKUP/COURSE/HEADER',
                array(
                    'newfields' => array(
                        'summaryformat'          => 1,
                        'legacyfiles'            => 2,
            ...
            new convert_path(
                'course_header_category', '/LION_BACKUP/COURSE/HEADER/CATEGORY',
                array(
                    'newfields' => array(
                        'description' => null,
                    )
    ...
    public function on_course_header_end() {
        ...
        $this->course['summary'] = lion1_converter::migrate_referenced_files($this->course['summary'], $fileman);
 ...
 */
class lion1_course_outline_handler extends lion1_xml_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path('course_sections', '/LION_BACKUP/COURSE/SECTIONS'),
            new convert_path(
                'course_section', '/LION_BACKUP/COURSE/SECTIONS/SECTION',
                array(
                    'newfields' => array(
                        'name'          => null,
                        'summaryformat' => 1,
            ...
            new convert_path(
                'course_module', '/LION_BACKUP/COURSE/SECTIONS/SECTION/MODS/MOD',
                array(
                    'newfields' => array(
                        'completion'                => 0,
            ...
            new convert_path('course_modules', '/LION_BACKUP/COURSE/MODULES'),
            // todo new convert_path('course_module_roles_overrides', '/LION_BACKUP/COURSE/SECTIONS/SECTION/MODS/MOD/ROLES_OVERRIDES'),
            // todo new convert_path('course_module_roles_assignments', '/LION_BACKUP/COURSE/SECTIONS/SECTION/MODS/MOD/ROLES_ASSIGNMENTS'),
        );
    }
    ...
    public function on_course_section_end() {
        ...
        $this->currentsection['summary'] = lion1_converter::migrate_referenced_files($this->currentsection['summary'], $fileman);
    ...
    public function on_course_modules_end() {
        ...
        foreach ($this->converter->get_stash('modnameslist') as $modname) {
            ...
            foreach ($modinfo['instances'] as $modinstanceid => $modinstance) {
                ...
                $this->write_xml('grade_letters', array()); // no grade_letters in module context in Lion 1.9
 ...
 */
class lion1_roles_definition_handler extends lion1_xml_handler {

    /**
     * Where the roles are defined in the source lion.xml
     */
    public function get_paths() {
        return array(
            new convert_path('roles', '/LION_BACKUP/ROLES'),
            new convert_path(
                'roles_role', '/LION_BACKUP/ROLES/ROLE',
                array(
                    'newfields' => array(
                        'description'   => '',
                        'sortorder'     => 0,
    ...
    /**
     * If there are any roles defined in lion.xml, convert them to roles.xml
     */
    ...
    public function on_roles_end() {
        ...
        if (!$this->has_xml_writer()) {
            // no roles defined in lion.xml so {link self::process_roles_role()}
            // was never executed
            $this->open_xml_writer('roles.xml');
...
/**
 * Handles the conversion of the question bank included in the lion.xml file
 */
class lion1_question_bank_handler extends lion1_xml_handler {
    ...
    /** @var lion1_file_manager instance used to convert question images */
    ...
    public function get_paths() {
        ...
        $paths = array(
            new convert_path('question_categories', '/LION_BACKUP/COURSE/QUESTION_CATEGORIES'),
            new convert_path(
                'question_category', '/LION_BACKUP/COURSE/QUESTION_CATEGORIES/QUESTION_CATEGORY',
                array(
                    'newfields' => array(
                        'infoformat' => 0
                    )
                )),
            new convert_path('question_category_context', '/LION_BACKUP/COURSE/QUESTION_CATEGORIES/QUESTION_CATEGORY/CONTEXT'),
            new convert_path('questions', '/LION_BACKUP/COURSE/QUESTION_CATEGORIES/QUESTION_CATEGORY/QUESTIONS'),
            // the question element must be grouped so we can re-dispatch it to the qtype handler as a whole
            new convert_path('question', '/LION_BACKUP/COURSE/QUESTION_CATEGORIES/QUESTION_CATEGORY/QUESTIONS/QUESTION', array(), true),
        );
        ...
        foreach (array_keys($subpaths) as $subpath) {
            ...
            $path = '/LION_BACKUP/COURSE/QUESTION_CATEGORIES/QUESTION_CATEGORY/QUESTIONS/QUESTION/'.$subpath;
    ...
    public function process_question_category_context($data) {
        ...
        case 'course':
            ...
            break;
        case 'coursecategory':
            // this is a bit hacky. the source lion.xml defines COURSECATEGORYLEVEL as a distance
    ...
    public function process_question(array $data, array $raw) {
        ...
        $data['questiontext'] = lion1_converter::migrate_referenced_files($data['questiontext'], $this->fileman);

        // Migrate files in generalfeedback.
        $this->fileman->filearea  = 'generalfeedback';
        $data['generalfeedback'] = lion1_converter::migrate_referenced_files($data['generalfeedback'], $this->fileman);
        ...
        foreach (array(
            ...
            if (!array_key_exists($fieldname, $data)) {
                throw new lion1_convert_exception('missing_common_question_field', $fieldname);
            }
     ...
     * @return array|lion1_qtype_handler|bool
     */
    protected function get_qtype_handler($qtype) {
        ...
        if (is_null($this->qtypehandlers)) {
            ...
            foreach (core_component::get_plugin_list('qtype') as $qtypename => $qtypelocation) {
                $filename = $qtypelocation.'/backup/lion1/lib.php';
                if (file_exists($filename)) {
                    $classname = 'lion1_qtype_'.$qtypename.'_handler';
                    require_once($filename);
                    if (!class_exists($classname)) {
                        throw new lion1_convert_exception('missing_handler_class', $classname);
                    }
...
/**
 * Handles the conversion of the scales included in the lion.xml file
 */
class lion1_scales_handler extends lion1_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path('scales', '/LION_BACKUP/COURSE/SCALES'),
            new convert_path(
                'scale', '/LION_BACKUP/COURSE/SCALES/SCALE',
                array(
                    'renamefields' => array(
                        'scaletext' => 'scale',
                    ),
    ...
    public function process_scale(array $data, array $raw) {
        ...
        $this->fileman->itemid = $data['id'];
        $data['description'] = lion1_converter::migrate_referenced_files($data['description'], $this->fileman);
 ...
 */
class lion1_outcomes_handler extends lion1_xml_handler {

    /** @var lion1_file_manager instance used to convert images embedded into outcome descriptions */
    protected $fileman = null;
    ...
    public function get_paths() {
        return array(
            new convert_path('gradebook_grade_outcomes', '/LION_BACKUP/COURSE/GRADEBOOK/GRADE_OUTCOMES'),
            new convert_path(
                'gradebook_grade_outcome', '/LION_BACKUP/COURSE/GRADEBOOK/GRADE_OUTCOMES/GRADE_OUTCOME',
                array(
                    'addfields' => array(
...
/**
 * Handles the conversion of the gradebook structures in the lion.xml file
 */
class lion1_gradebook_handler extends lion1_xml_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path('gradebook', '/LION_BACKUP/COURSE/GRADEBOOK'),
            new convert_path('gradebook_grade_letter', '/LION_BACKUP/COURSE/GRADEBOOK/GRADE_LETTERS/GRADE_LETTER'),
            new convert_path(
                'gradebook_grade_category', '/LION_BACKUP/COURSE/GRADEBOOK/GRADE_CATEGORIES/GRADE_CATEGORY',
                array(
                    'addfields' => array(
            ...
            new convert_path('gradebook_grade_item', '/LION_BACKUP/COURSE/GRADEBOOK/GRADE_ITEMS/GRADE_ITEM'),
            new convert_path('gradebook_grade_item_grades', '/LION_BACKUP/COURSE/GRADEBOOK/GRADE_ITEMS/GRADE_ITEM/GRADE_GRADES'),
        );
    }
     ...
     *
     * This should not be needed actually as the lion.xml contains just one GRADEBOOK
     ...
     *
     * In Lion 1.9, all grade_letters are from course context only. Therefore
     * we put them here.
     */
     ...
     *
     * Lion 1.9 backup does not store the grade_category's depth and path. This method is used
    ...
    protected function calculate_category_path($categoryid) {
        ...
        if (!array_key_exists($categoryid, $this->categoryparent)) {
            throw new lion1_convert_exception('gradebook_unknown_categoryid', null, $categoryid);
        }
        ...
        while (!is_null($parent)) {
            ...
            if (in_array($parent, $path)) {
                throw new lion1_convert_exception('circular_reference_in_categories_tree');
            }
        }
 ...
 */
abstract class lion1_plugin_handler extends lion1_xml_handler {
     ...
     */
    public function __construct(lion1_converter $converter, $plugintype, $pluginname) {
 ...
 */
abstract class lion1_qtype_handler extends lion1_plugin_handler {

    /** @var lion1_question_bank_handler */
     ...
     *
     * @param array $answers as parsed by the grouped parser in lion.xml
    ...
    protected function migrate_files($text, $component, $filearea, $itemid) {
        ...
        $text = lion1_converter::migrate_referenced_files($text, $fileman);
        return $text;
    }
     ...
     *
     * This structure is not present in lion.xml, we create a new artificial one here.
     *
    ...
    public function __construct(lion1_question_bank_handler $qbankhandler, $qtype) {
    ...
    final public function get_paths() {
        throw new lion1_convert_exception('qtype_handler_get_paths');
    }
    ...
    final protected function open_xml_writer($filename) {
        throw new lion1_convert_exception('opening_xml_writer_forbidden');
    }
     ...
     *
     * @param array $old the parsed answer array in lion.xml
    ...
    private function convert_answer(array $old, $qtype) {
        ...
        if ($CFG->texteditors !== 'textarea') {
            ...
            $new['feedbackformat'] = FORMAT_LION;
        }
 ...
 */
abstract class lion1_mod_handler extends lion1_plugin_handler {
     ...
     * {@link lion1_course_outline_handler::process_course_module()}
     *
 ...
 */
abstract class lion1_resource_successor_handler extends lion1_mod_handler {

    /**
     * Resource successors do not attach to paths themselves, they are called explicitely
     * by lion1_mod_resource_handler
     *
     * @return array
     */
    final public function get_paths() {
     ...
     *
     * Called by {@link lion1_mod_resource_handler::process_resource()}
     *
 ...
 */
abstract class lion1_block_handler extends lion1_plugin_handler {

    public function get_paths() {
        $blockname = strtoupper($this->pluginname);
        return array(
            new convert_path('block', "/LION_BACKUP/COURSE/BLOCKS/BLOCK/{$blockname}"),
        );
    }
    ...
    protected function write_roles_xml($newdata, $data) {
        // This is an empty shell, as the lion1 converter doesn't handle user data.
 ...
 */
class lion1_block_generic_handler extends lion1_block_handler {
 ...
 */
abstract class lion1_submod_handler extends lion1_plugin_handler {

    /** @var lion1_mod_handler */
    protected $parenthandler;

    /**
     * @param lion1_mod_handler $parenthandler the handler of a module we are subplugin of
     * @param string $subplugintype the type of the subplugin
     * @param string $subpluginname the name of the subplugin
     */
    public function __construct(lion1_mod_handler $parenthandler, $subplugintype, $subpluginname) {
