...
global $CFG;
require_once($CFG->dirroot . '/backup/converter/lion1/lib.php');


class core_backup_lion1_converter_testcase extends advanced_testcase {

    /** @var string the name of the directory containing the unpacked Lion 1.9 backup */
    protected $tempdir;
    ...
    protected function setUp() {
        ...
        copy(
            "$CFG->dirroot/backup/converter/lion1/tests/fixtures/lion.xml",
            "$CFG->tempdir/backup/$this->tempdir/lion.xml"
        );
        copy(
            "$CFG->dirroot/backup/converter/lion1/tests/fixtures/icon.gif",
    ...
    public function test_detect_format() {
        $detected = lion1_converter::detect_format($this->tempdir);
        $this->assertEquals(backup::FORMAT_LION1, $detected);
    }

    public function test_convert_factory() {
        $converter = convert_factory::get_converter('lion1', $this->tempdir);
        $this->assertInstanceOf('lion1_converter', $converter);
    }

    public function test_stash_storage_not_created() {
        $converter = convert_factory::get_converter('lion1', $this->tempdir);
        $this->setExpectedException('lion1_convert_storage_exception');
    ...
    public function test_stash_requiring_empty_stash() {
        ...
        $converter->set_stash('tempinfo', 12);
        $this->setExpectedException('lion1_convert_empty_storage_exception');
        try {
            ...
            throw new lion1_convert_empty_storage_exception('rethrowing');
        }
    }
    ...
    public function test_convert_path_implicit_recipes() {
        ...
        $this->assertEquals('lionr', $data['martin']);
...
as it is parsed from the backup file. <br /><br /><img border="0" width="110" vspace="0" hspace="0" height="92" title="News" alt="News" src="$@FILEPHP@$$@SLASH@$pics$@SLASH@$news.gif" /><a href="$@FILEPHP@$$@SLASH@$pics$@SLASH@$news.gif$@FORCEDOWNLOAD@$">download image</a><br />
    ...
    <br /><a href=\'$@FILEPHP@$$@SLASH@$MANUAL.DOC$@FORCEDOWNLOAD@$\'>download manual</a><br />';
        ...
        $files = lion1_converter::find_referenced_files($text);
        ...
        $text = lion1_converter::rewrite_filephp_usage($text, array('/pics/news.gif', '/another/file/notused.txt'));
...
<a href="$@FILEPHP@$$@SLASH@$illegal pics%2Bmovies$@SLASH@$romeo%2Bjuliet.avi">Download the full AVI for free! (only plus)</a>';
        ...
        $text = lion1_converter::rewrite_filephp_usage($text, $files);
...
<a href="$@FILEPHP@$$@SLASH@$illegal pics%2Bmovies$@SLASH@$romeo%2Bjuliet.avi">Download the full AVI for free! (only plus)</a>', $text);
    ...
    public function test_question_bank_conversion() {
        ...
        copy(
            "$CFG->dirroot/backup/converter/lion1/tests/fixtures/questions.xml",
            "$CFG->tempdir/backup/$this->tempdir/lion.xml"
        );
        $converter = convert_factory::get_converter('lion1', $this->tempdir);
        $converter->convert();
