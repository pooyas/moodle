...
/**
 * Converter of Lion 1.9 backup into Lion 2.x format
 */
class lion1_converter extends base_converter {

    /** @var progressive_parser lion.xml file parser */
    protected $xmlparser;

    /** @var lion1_parser_processor */
    ...
    public function log($message, $level, $a = null, $depth = null, $display = false) {
        parent::log('(lion1) '.$message, $level, $a, $depth, $display);
    }

    /**
     * Detects the Lion 1.9 format of the backup directory
     *
     * @param string $tempdir the name of the backup directory
     * @return null|string backup::FORMAT_LION1 if the Lion 1.9 is detected, null otherwise
     */
    public static function detect_format($tempdir) {
        global $CFG;

        $filepath = $CFG->tempdir . '/backup/' . $tempdir . '/lion.xml';
        if (file_exists($filepath)) {
            ...
            if (strpos($first_chars,'<?xml version="1.0" encoding="UTF-8"?>') !== false and
                strpos($first_chars,'<LION_BACKUP>') !== false and
                strpos($first_chars,'<INFO>') !== false) {

                return backup::FORMAT_LION1;
            }
        }
    ...
    protected function init() {
        ...
        $this->xmlparser->set_file($this->get_tempdir_path() . '/lion.xml');
        $this->log('setting xml processor', backup::LOG_DEBUG, null, 1);
        $this->xmlprocessor = new lion1_parser_processor($this);
        $this->xmlparser->set_processor($this->xmlprocessor);

        // make sure that MOD and BLOCK paths are visited
        $this->xmlprocessor->add_path('/LION_BACKUP/COURSE/MODULES/MOD');
        $this->xmlprocessor->add_path('/LION_BACKUP/COURSE/BLOCKS/BLOCK');

        // register the conversion handlers
        foreach (lion1_handlers_factory::get_handlers($this) as $handler) {
    ...
    protected function execute() {
        ...
        $this->log('parsing lion.xml starts', backup::LOG_DEBUG);
        $this->xmlparser->process();
        $this->log('parsing lion.xml done', backup::LOG_DEBUG);
     ...
     */
    protected function register_handler(lion1_handler $handler, array $elements) {
        ...
        foreach ($elements as $element) {
            ...
            $path = preg_replace('/^\/LION_BACKUP\/COURSE\/MODULES\/MOD\/(\w+)\//', '/LION_BACKUP/COURSE/MODULES/MOD/', $path);
            $path = preg_replace('/^\/LION_BACKUP\/COURSE\/BLOCKS\/BLOCK\/(\w+)\//', '/LION_BACKUP/COURSE/BLOCKS/BLOCK/', $path);
    ...
    public function process_chunk($data) {
        ...
        if ($path === '/LION_BACKUP/COURSE/MODULES/MOD') {
            $this->currentmod = strtoupper($data['tags']['MODTYPE']);
            $path = '/LION_BACKUP/COURSE/MODULES/MOD/' . $this->currentmod;

        } else if (strpos($path, '/LION_BACKUP/COURSE/MODULES/MOD') === 0) {
            $path = str_replace('/LION_BACKUP/COURSE/MODULES/MOD', '/LION_BACKUP/COURSE/MODULES/MOD/' . $this->currentmod, $path);
        }

        // expand the BLOCK paths so that they contain the module name
        if ($path === '/LION_BACKUP/COURSE/BLOCKS/BLOCK') {
            $this->currentblock = strtoupper($data['tags']['NAME']);
            $path = '/LION_BACKUP/COURSE/BLOCKS/BLOCK/' . $this->currentblock;

        } else if (strpos($path, '/LION_BACKUP/COURSE/BLOCKS/BLOCK') === 0) {
            $path = str_replace('/LION_BACKUP/COURSE/BLOCKS/BLOCK', '/LION_BACKUP/COURSE/BLOCKS/BLOCK/' . $this->currentblock, $path);
        }
     ...
     * /LION_BACKUP/COURSE/MODULES/MOD/WORKSHOP because the <MOD> must
     ...
     * /LION_BACKUP/COURSE/MODULES/MOD/WORKSHOP/SUBMISSIONS because it is
    ...
    public function set_stash($stashname, $info, $itemid = 0) {
        try {
            ...
            throw new lion1_convert_storage_exception('unable_to_restore_stash', null, $e->getMessage());
        }
    }
     ...
     * @param string $stashname name of the stash
     * @param int $itemid optional id for multiple infos within the same stashname
     * @throws lion1_convert_empty_storage_exception if the info has not been stashed previously
    ...
    public function get_stash($stashname, $itemid = 0) {
        ...
        if (empty($record)) {
            throw new lion1_convert_empty_storage_exception('required_not_stashed_data', array($stashname, $itemid));
        } else {
    ...
    public function get_stash_or_default($stashname, $itemid = 0, $default = null) {
        ...
        } catch (lion1_convert_empty_storage_exception $e) {
            return $default;
        }
    }
     ...
     *
     * Lion 1.9 backups do not contain any context information. But we need them
     * in Lion 2.x format so here we generate fictive context id for every given
    ...
    public function get_file_manager($contextid = null, $component = null, $filearea = null, $itemid = 0, $userid = null) {
        return new lion1_file_manager($this, $contextid, $component, $filearea, $itemid, $userid);
    }
     ...
     * @return lion1_inforef_manager
     */
    public function get_inforef_manager($name, $id = 0) {
        return new lion1_inforef_manager($this, $name, $id);
    }
     ...
     * @param lion1_file_manager $fileman file manager to use for the file migration
     * @return string the original $text with $@FILEPHP@$ references replaced with the new @@PLUGINFILE@@
     */
    public static function migrate_referenced_files($text, lion1_file_manager $fileman) {
        ...
        if (!empty($files)) {
            foreach ($files as $file) {
                ...
                } catch (lion1_convert_exception $e) {
    ...
    public static function find_referenced_files($text) {
        ...
        if ($result === false) {
            throw new lion1_convert_exception('error_while_searching_for_referenced_files');
        }
        if ($result == 0) {
            return $files;
        }
 ...
 */
class lion1_convert_exception extends convert_exception {
}


/**
 * Exception thrown by the temporary storage subsystem of lion1_converter
 */
class lion1_convert_storage_exception extends lion1_convert_exception {
}
 ...
 * Exception thrown by the temporary storage subsystem of lion1_converter
 */
class lion1_convert_empty_storage_exception extends lion1_convert_exception {
}


/**
 * XML parser processor used for processing parsed lion.xml
 */
class lion1_parser_processor extends grouped_parser_processor {
 ...
 */
class lion1_xml_transformer extends xml_contenttransformer {
    ...
    public function process($content) {
        ...
        if (is_array($content)) {
            debugging('Lion1 XML transformer should not process arrays but plain content always', DEBUG_DEVELOPER);
            ...
            debugging('Lion1 XML transformer should not process objects but plain content always', DEBUG_DEVELOPER);
...
class convert_path {
     ...
     * {@line lion1_course_outline_handler} is a good example that uses all of these.
     *
 ...
 */
class convert_path_exception extends lion_exception {
 ...
 *
 * The files in Lion 1.9 backup are stored in moddata, user_files, group_files,
 * course_files and site_files folders.
 */
class lion1_file_manager implements loggable {

    /** @var lion1_converter instance we serve to */
    public $converter;
     ...
     *
     * @param lion1_converter $converter the converter instance we serve to
     ...
     * @param int $userid initial user id of the files being migrated
     */
    public function __construct(lion1_converter $converter, $contextid = null, $component = null, $filearea = null, $itemid = 0, $userid = null) {
    ...
    public function migrate_file($sourcepath, $filepath = '/', $filename = null, $sortorder = 0, $timecreated = null, $timemodified = null) {
        ...
        if ($sourcepath !== clean_param($sourcepath, PARAM_PATH)) {
            throw new lion1_convert_exception('file_invalid_path', $sourcepath);
        }

        $sourcefullpath = $this->basepath.'/'.$sourcepath;

        if (!is_readable($sourcefullpath)) {
            throw new lion1_convert_exception('file_not_readable', $sourcefullpath);
        }
        ...
        if (core_text::strlen($filepath) > 255) {
            throw new lion1_convert_exception('file_path_longer_than_255_chars');
        }
        ...
        if ($filename === '') {
            throw new lion1_convert_exception('unsupported_chars_in_filename');
        }
    ...
    public function migrate_directory($rootpath, $relpath='/') {
        ...
        if (substr($rootpath, -1) === '/') {
            debugging('lion1_file_manager::migrate_directory() expects $rootpath without the trailing slash', DEBUG_DEVELOPER);
        ...
        foreach ($items as $item) {
            ...
            if ($item->isLink()) {
                throw new lion1_convert_exception('unexpected_symlink');
            }
    ...
    protected function add_file_to_pool($pathname) {
        ...
        if (!is_readable($pathname)) {
            throw new lion1_convert_exception('file_not_readable');
        }
        ...
        if (file_exists($hashfile)) {
            if (filesize($hashfile) !== $filesize) {
                ...
                throw new lion1_convert_exception('same_hash_different_size');
            }
            $newfile = false;
            ...
            if (!copy($pathname, $hashfile)) {
                throw new lion1_convert_exception('unable_to_copy_file');
            }

            if (filesize($hashfile) !== $filesize) {
                throw new lion1_convert_exception('filesize_different_after_copy');
            }
        }
...
class lion1_inforef_manager {
     ...
     *
     * @param lion1_converter $converter
     * @param string $name the name of the annotator (like course, section, activity, block)
     * @param int $id the id of the annotator if required
     */
    public function __construct(lion1_converter $converter, $name, $id = 0) {
        $this->annotator   = $name;
        $this->annotatorid = $id;
    }
