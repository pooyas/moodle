...
class quiz_overview_report extends quiz_attempts_report {
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
            ...
            if (!$table->is_downloading()) {
                ...
                if (has_capability('mod/quiz:regrade', $this->context)) {
                    ...
                    $displayurl = new lion_url($options->get_url(), array('sesskey' => sesskey()));
        ...
        if (!$table->is_downloading() && $options->usercanseegrades) {
            ...
            if ($currentgroup && $groupstudents) {
                ...
                if ($DB->record_exists_select('quiz_grades', "userid $usql AND quiz = ?",
                        $params)) {
                    $imageurl = new lion_url('/mod/quiz/report/overview/overviewgraph.php',
     ...
     * @param lion_url $nexturl where to send the user after the regrade.
