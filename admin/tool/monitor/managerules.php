...
if (!empty($action) && $action == 'changestatus') {
    ...
    redirect(new lion_url('/admin/tool/monitor/managerules.php', array('courseid' => 0)));
}
    ...
    switch ($action) {
        ...
        case 'delete':
            if ($rule->can_manage_rule()) {
                $confirmurl = new lion_url($CFG->wwwroot. '/admin/tool/monitor/managerules.php',
                    array('ruleid' => $ruleid, 'courseid' => $courseid, 'action' => 'delete',
                        'confirm' => true, 'sesskey' => sesskey()));
                $cancelurl = new lion_url($CFG->wwwroot. '/admin/tool/monitor/managerules.php',
                ...
                throw new lion_exception('rulenopermissions', 'tool_monitor', $manageurl, $action);
            }
            break;
        default:
    }
...
if ($status) {
    if (has_capability('tool/monitor:managetool', context_system::instance())) {
        ...
        $disableurl = new lion_url("/admin/tool/monitor/managerules.php",
    ...
    if (has_capability('tool/monitor:managetool', context_system::instance())) {
        $enableurl = new lion_url("/admin/tool/monitor/managerules.php",
...
if (has_capability('tool/monitor:subscribe', $context)) {
    $manageurl = new lion_url("/admin/tool/monitor/index.php", array('courseid' => $courseid));
